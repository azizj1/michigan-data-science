...
def q2(tokens):
    ...
    return filtered_count / len(tokens)
...
def q4(text):
    freq = FreqDist(text)
    return [word for word in freq.keys() if len(word) > 5 and freq[word] > 150]
...
def q6(text):
    freq = FreqDist(text)
    return list(filter(lambda x: x[0].isalpha() and x[1] > 2000, freq.most_common()))
